 ...
 *
 * Every API failure leaves through here as one envelope, matching the success
 * shape produced by App\Traits\ApiResponse:
 *
 *     {"success": false, "message": "...", "errors": {...}|null, "code": 422}
 *
 * Chain of Responsibility, collapsed into a single match(): each arm claims
 * the exceptions it recognises and everything else falls through to the
 * catch-all, so adding a translation never touches the existing ones
 * (Open/Closed). Clients parse one shape instead of guessing between
 * Laravel's HTML page, its validation JSON, and a bare 500.
 *
 * OWASP A10 (Mishandling of Exceptional Conditions) and A02: an unrecognised
 * exception becomes a generic 500. Class name, message, file and line are
 * attached only when APP_DEBUG is on — those strings routinely carry absolute
 * paths, SQL fragments and credentials, and APP_DEBUG must be false in
 * production.
 *
...
{
    /**
     * Laravel 11+: called from bootstrap/app.php inside withExceptions().
     *
    ...
    /**
     * Laravel 10: called from the application's own exception handler.
     */
